...
const API_URL = "https://r4ver.com/eq/api/";

const request = (method, path, options = {} ) => {
    const dataType = (options.dataType ? options.dataType : "application/json");
    const version = (options.version ? options.version : "v1");
    return new Promise( (resolve, reject) => {
        fetch(`${API_URL}${version}/${path}${options.qs ? `${querystring.stringify(options.qs)}` : ''}`, {
            method: method,
            mode: "cors",
            headers: {
